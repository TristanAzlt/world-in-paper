...
contract WorldInPaper is ReceiverTemplate {
    ...
    error InvalidStartingWIPBalance(uint256 provided, uint256 minAllowed, uint256 maxAllowed);
    error InvalidMaxPlayers(uint16 provided, uint16 minAllowed, uint16 maxAllowed);
    ...
    error GameAlreadyStarted(uint256 gameId, uint256 startTime, uint256 currentTimestamp);
    ...
    error GameNotStarted(uint256 gameId, uint256 startTime, uint256 currentTimestamp);
    error GameEnded(uint256 gameId, uint256 endTime, uint256 currentTimestamp);
    error GameNotEnded(uint256 gameId, uint256 endTime, uint256 currentTimestamp);
    ...
    error InsufficientWIPBalance(uint256 gameId, address player, uint256 balance, uint256 required);
    error InsufficientTokenBalance(uint256 gameId, address player, uint256 balance, uint256 required);
    ...
    event GameJoined(uint256 indexed gameId, address indexed player, uint16 playerCount);
    event TradeRecorded(
        uint256 indexed gameId,
    ...
    event SettlementRequest(
        uint256 indexed tradeId, uint256 indexed gameId, string asset_address, Origin origin, bool isBuy, uint256 amount
    );
    event TradeSettled(
        uint256 indexed tradeId,
        address indexed trader,
    ...
    event GameClaimed(uint256 indexed gameId, address indexed player, uint256 payout, uint256 rank);
    ...
    function createGame(
        ...
        _validateCreateParams(entryAmount, startingWIPBalance, maxPlayers, startTime, endTime);
        ...
        emit GameCreated(gameId, creator, entryAmount, maxPlayers, startTime, endTime);

        _joinGame(gameId, creator, worldId);
    }

    function joinGame(uint256 gameId, WorldIdVerification calldata worldId) external {
    ...
    function submitTrade(
        ...
        emit SettlementRequest(tradeId, gameId, asset_address, origin, isBuy, amountIn);
    }
    ...
    function hasClaimed(uint256 gameId, address player) external view returns (bool) {
        if (!_games[gameId].exists) {
            revert GameNotFound(gameId);
        }
    ...
    function getPlayerWipBalance(uint256 gameId, address player) external view returns (uint256) {
        Game storage game = _games[gameId];
        if (!game.exists) {
            revert GameNotFound(gameId);
        }
    ...
    function getGameTrades(uint256 gameId) external view returns (Trade[] memory) {
        if (!_games[gameId].exists) {
            revert GameNotFound(gameId);
        }
    ...
    function getTradeToSettle(uint256 tradeId) external view returns (TradeToSettle memory) {
    ...
    function _validateCreateParams(
        ...
        if (startingWIPBalance < MIN_STARTING_WIP_BALANCE || startingWIPBalance > MAX_STARTING_WIP_BALANCE) {
            revert InvalidStartingWIPBalance(startingWIPBalance, MIN_STARTING_WIP_BALANCE, MAX_STARTING_WIP_BALANCE);
        }
    ...
    function _joinGame(uint256 gameId, address player, WorldIdVerification calldata worldId) internal {
        Game storage game = _games[gameId];
        if (!game.exists) {
            revert GameNotFound(gameId);
        }
        ...
        bool success = USDC.transferFrom(player, address(this), game.entryAmount);
        if (!success) {
    ...
    function _getSortedPlayers(Game storage game) internal view returns (address[] memory sortedPlayers) {
        ...
        for (uint256 i = 0; i < playersLength;) {
            ...
            for (uint256 j = i + 1; j < playersLength;) {
                if (_isHigherRank(game, sortedPlayers[j], sortedPlayers[bestIdx])) {
                    bestIdx = j;
                }
    ...
    function _getPlayerRank(Game storage game, address player) internal view returns (uint256 rank) {
    ...
    function _computeClaimPayout(Game storage game, address player)
        internal
        view
        returns (uint256 payout, uint256 rank)
    {
    ...
    function _isHigherRank(Game storage game, address candidate, address current) internal view returns (bool) {
    ...
    function _processReport(bytes calldata report) internal override {
        ...
        if (tradeToSettle.isBuy) {
            amountOut = (tradeToSettle.amountIn * EXECUTION_PRICE_PRECISION) / executionPrice;
        } else {
            amountOut = (tradeToSettle.amountIn * executionPrice) / EXECUTION_PRICE_PRECISION;
        }
