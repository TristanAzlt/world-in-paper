...
var USDC = {
  ...
  [ORIGIN.WORLD]: "0x79A02482A880bCE3F13e09Da970dC34db4CD24d1"
};
var CHAIN_ID = {
  [ORIGIN.BASE]: 8453,
  [ORIGIN.ETHEREUM]: 1,
...
var getSolanaTokenDecimals = (sendRequester, mintAddress) => {
  ...
  if (resp.statusCode !== 200)
    throw new Error(`Solana RPC HTTP ${resp.statusCode}`);
  const data = JSON.parse(Buffer.from(resp.body).toString("utf-8"));
...
var fetchJupiterQuote = (sendRequester, mintAddress, isBuy, amount) => {
  ...
  if (resp.statusCode !== 200)
    throw new Error(`Jupiter HTTP ${resp.statusCode}`);
  const data = JSON.parse(Buffer.from(resp.body).toString("utf-8"));
...
var onSettlementRequest = (runtime2, log) => {
  ...
  const gameId = BigInt(bytesToHex(log.topics[2]));
  ...
  runtime2.log(`SettlementRequest: game=${gameId} trade=${tradeId} asset=${assetId} origin=${origin} isBuy=${isBuy} amount=${amount}`);
  const httpClient = new cre.capabilities.HTTPClient;
  const quoteResult = httpClient.sendRequest(runtime2, fetchQuote, ConsensusAggregationByFields({
    price: median
  }))(assetId, Number(origin), isBuy, amount.toString(), uniswapApiKey).result();
...
function initWorkflow(config) {
  ...
  const evmClient = new cre.capabilities.EVMClient(network282.chainSelector.selector);
  return [
    cre.handler(evmClient.logTrigger(logTriggerConfig({
